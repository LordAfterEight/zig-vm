...
pub const GPU = struct {
    ...
    pub fn init() !GPU {
        ...
        const font = sfml.sfFont_createFromFile("src/res/fonts/BigBlueTerminal/BigBlueTerm437NerdFont-Regular.ttf");
    ...
    pub fn update(self: *GPU) void {
        ...
        switch (self.draw_mode) {
            true => {
                std.log.info("Found character: {}", .{instruction});
                const char: u8 = @intCast(instruction);
                switch (char) {
    ...
    pub fn render(self: *GPU) void {
        ...
        for (0..self.frame_buffer[0].len) |y| {
            for (0..self.frame_buffer.len) |x| {
                ...
                sfml.sfText_setString(text, &str[0]);
                sfml.sfText_setPosition(text, sfml.sfVector2f{
                    .x = @floatFromInt(x * (self.font_size - 4)),
                    .y = @floatFromInt(y * (self.font_size)),
                });
                sfml.sfRenderWindow_drawText(self.window, text, null);
            }
        }
